...
export class BookImportComponent {
  protected bookForm = new FormGroup({
    [FormGroupKey.file]: new FormControl<File | null>(
      null,
      Validators.required,
    ),
  ...
  get chapters() {
    return this.bookForm.controls[
      FormGroupKey.chapters
    ] as FormArray<FormGroup>;
  }

  protected removeChapter(chapterFg: FormGroup) {
    const chapterFormArray = this.bookForm.controls[
      FormGroupKey.chapters
    ] as FormArray<FormGroup>;
  ...
  protected onFileSelected($event: Event): void {
    ...
    this.bookForm.controls[FormGroupKey.file].setValue(
    ...
    if (!!this.bookForm.controls[FormGroupKey.file].value) {
      ...
      let chapters = new Array(chapterFormArray.length)
        ...
        .map(
          (chapterFg) =>
            ({
              title: chapterFg.controls[FormGroupKey.chapterHeading].value,
            }) as ChapterHeading,
        );
      // for testing only
  ...
  private frankensteinChapters(): ChapterHeading[] {
    ...
    chapters.push({
      title: '*** END OF THE PROJECT GUTENBERG EBOOK',
    } as ChapterHeading);
    return chapters;
  }
}

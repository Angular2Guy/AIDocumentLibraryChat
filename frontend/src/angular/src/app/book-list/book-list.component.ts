  ...
  ngOnInit(): void {
    this.myControl.valueChanges
      .pipe(
        debounceTime(300),
        tap((myValue) => {
          if (typeof myValue === 'object') this.selBook = myValue;
        }),
        switchMap((myValue) =>
          typeof myValue === 'string'
            ? this.documentService
                .getBooksByTitleAuthor(myValue)
                .pipe(catchError((err) => []))
            : of([]),
        ),
      )
      .subscribe({
        next: (myArr) => (this.filteredOptions = myArr),
        error: (myErr) => (this.filteredOptions = [] as Book[]),
      });
  }

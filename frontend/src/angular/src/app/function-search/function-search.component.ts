...
export class FunctionSearchComponent {
    ...
    this.functionSearchService
      ...
      .subscribe((value) =>
        this.resultFormatControl.value === this.resultFormats[0]
          ...
          : (this.responseJson = this.addToDataSource(
              this.mapResult(
                value.jsonResult ||
  ...
  private mapResult(jsonResults: JsonResult[]): TreeNode[] {
    const createChildren = (books: JsonBook[]) =>
      books.map(
        (value) => ({ value1: value.title, value2: value.summary }) as TreeNode,
      );
    const rootNode = jsonResults.map(
      (myValue) =>
        ({
          value1: myValue.author,
          value2: '',
          children: createChildren(myValue.books),
        }) as TreeNode,
    );
    return rootNode;
  }

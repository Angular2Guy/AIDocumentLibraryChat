 ...
 */
package ch.xxx.aidoclibchat.architecture;
...
import ch.xxx.aidoclibchat.architecture.MyArchitectureTests.DoNotIncludeGenerated;
import jakarta.annotation.PostConstruct;
import net.javacrumbs.shedlock.spring.annotation.SchedulerLock;

@AnalyzeClasses(packages = "ch.xxx.aaidoclibchat", importOptions = { DoNotIncludeTests.class,
		DoNotIncludeGenerated.class })
public class MyArchitectureTests {
	private static final Logger LOGGER = LoggerFactory.getLogger(MyArchitectureTests.class);
	...
	private JavaClasses importedClasses = new ClassFileImporter()
			.withImportOptions(List.of(new DoNotIncludeTests(), new DoNotIncludeGenerated()))
			.importPackages("ch.xxx.aidoclibchat");
...
//			.adapter("client", "..adapter.client..")
	...
	@Test
	public void cyclesDomain() {
		SlicesRuleDefinition.slices().matching("..domain.(*)..").should().beFreeOfCycles();
	}

	@Test
	public void cyclesUseCases() {
		SlicesRuleDefinition.slices().matching("..usecase.(*)..").should().beFreeOfCycles();
	}

	@Test
	public void cyclesAdapter() {
		SlicesRuleDefinition.slices().matching("..adapter.(*)..").should().beFreeOfCycles();
	}
...
//	@Test
	...
	static final class DoNotIncludeGenerated implements ImportOption {
		private static final Pattern GENERATED_CLASSES_PATTERN = Pattern.compile(".+\\$\\d+\\.class$");
		private static final Pattern AOT_GENERATED_PATTERN = Pattern
				.compile(".*(__BeanDefinitions|SpringCGLIB\\$\\$0)\\.class$");
		private static final Pattern AOT_TEST_GENERATED_PATTERN = Pattern.compile(".*__TestContext.*\\.class$");

		@Override
		public boolean includes(Location location) {
			return !(location.matches(AOT_GENERATED_PATTERN) || location.matches(AOT_TEST_GENERATED_PATTERN) || location.matches(GENERATED_CLASSES_PATTERN));
		}
	}
}
